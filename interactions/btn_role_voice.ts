...
export const data = {
    customId: 'btn_role_voice',
    type: 'component'
}
export async function execute(interaction: ButtonInteraction) {
    const member = await (await client.guilds.fetch(interaction.guild?.id || '')).members.fetch(interaction.user.id)
    try {
        const role = await member.guild.roles.fetch('1240765855135174798')
        if(role) {
            if(member.roles.cache.has(role.id)) {
                member.roles.remove(role)
                interaction.reply({content: 'Your role has been removed.', ephemeral: true})
                await removeSkill(interaction.user.id, interaction.user.tag, 'Voice')
            } else {
                member.roles.add(role)
                interaction.reply({content: 'You have been given the Voice Acting role.', ephemeral: true})
